...
// Compat, remove me.
 typealias ParserError = MessageParser.Error

/**
 * Parses `Message` objects from ByteBuffers.
 *
 * The parser is tolerant, if a line fails to parse, it yields an error and
 * continues parsing.
 */
 struct MessageParser {
  // Note:  does not actually specify an encoding. Lets be ignorant and
  //       consider it UTF8 ;-)
    ...
    buffer.withUnsafeReadableBytes { bp in
      ...
      while !cursor.isEmpty {
        ...
        do {
          ...
          yield( ( error as? ParserError ?? .syntaxError, nil ) )
        }
    ...
    switch commandKey {
      case .string(let s):
        return Message(origin: origin,
                          command: try Command(s, arguments: args))
      case .int(let i):
        return Message(origin: origin,
                          command: try Command(i, arguments: args))
    }
  }
}
